...
use ubitwarden::{
    ...
    error::{Error, Result},
    session::{BwSession, BwSessionData},
};
...
impl UBWAgentSafe {
    ...
    pub async fn server() -> Result<Self> {
        ...
        let unix_stream = UnixStream::connect(socket_name).await?;
        let stream = EncryptedChannel::listen(unix_stream).await?;

        Ok(Self { stream })
    }

    pub async fn quit(&mut self) -> Result<bool> {
        let msg = ChannelRequest::Quit;
    ...
    pub async fn session_fetch(&mut self) -> Result<BwSessionData> {
        let msg = ChannelRequest::SessionFetch;

        let res = send_message(&mut self.stream, msg).await?;

        let ChannelResponse::SessionFetch(session) = res else {
            return Err(Error::InvalidCommandResponse);
        };
    ...
    pub async fn session_store(&mut self, session: &BwSessionData) -> Result<bool> {
        let msg = ChannelRequest::SessionStore(session.clone());
    ...
    pub async fn session_load(&mut self) -> Result<BwSessionData> {
        ...
        Ok(session.deref().clone())
    }

    //
    // Credentials
    //
    pub async fn delete_crentials(&mut self) -> Result<bool> {
